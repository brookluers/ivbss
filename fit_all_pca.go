...
func main() {
	...
	doc.SetLogFile("log_before_pca.txt")
	doc.Init()
	pdim := len(regxnames)
	...
	projCoefs_expand := make([][]float64, npc)
	...
	for j := 0; j < npc; j++ {
		...
		for k, na := range ivr.XNames() {
			x[vm_pc[na]] = projCoefs[j][k] 
		}
		projCoefs_expand[j] = x 
	}
	...
	doc_pc.SetLogFile("log_pc_doc.txt")
	doc_pc.Init()
	ndir := 5
	doc_pc.Fit(ndir)

	dirs0 := [][]float64{doc_pc.MeanDir(), doc_pc.CovDir(0), doc_pc.CovDir(1)}
	...
	fmt.Printf("vm = %v\n",vm)
	for j := 0; j < 3; j++ {
		x := make([]float64, len(vm)) // length will be longer than len(ivr_pc.XNames())
		for k, na := range ivr_pc.XNames() {
