...
for (curID in 1:maxID){
    h <- read.csv(sprintf("/scratch/stats_flux/luers/hmap_multi_cd1_cd2_%03d.txt", curID), header=T)
    ggsave(sprintf("hmap_cd1_cd2_%03d.pdf", curID), 
    		p_cd %+% h +
		ggtitle(paste("Driver", curID)),
		width = 6, height=5.5, units='in')
    h <- read.csv(sprintf("/scratch/stats_flux/luers/hmap_multi_meandir_cd1_%03d.txt", curID), header=T)
